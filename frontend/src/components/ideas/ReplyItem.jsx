...
export default function ReplyItem({ reply, ideaId, commentId, user, onEdit, onLike, onDislike, onDelete }) {
    ...
    <div className="group relative">
      <div className="bg-white rounded-xl p-5 shadow-sm hover:shadow-md transition-all duration-300 border border-gray-100 hover:border-gray-200">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold text-xs">
              {(user.username ? user.username.charAt(0) : (user.fullName ? user.fullName.charAt(0) : 'A')).toUpperCase()}
            </div>
            <div>
              <span className="font-semibold text-gray-900 text-sm">{user.username || user.fullName || 'Anonymous'}</span>
              ...
              <div className="flex gap-1">
                {!editing && (
                  <>
                    ...
                    <button
                      onClick={() => onDelete(ideaId, commentId, reply.replyId)}
        ...
        {editing ? (
          ...
          <div className="mb-3 text-gray-800 leading-relaxed text-sm">
            {reply.text}
          </div>
        )}
        {/* Engagement bar */}
